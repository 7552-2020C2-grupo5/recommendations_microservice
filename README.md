...
```bash
poetry run python recommendations-microservice/manage.py db migrate -m "migration message"
```

Remember to review them after creating them.

## Style guide
recommendations-microservice follows [PEP8](https://www.python.org/dev/peps/pep-0008/).
...
## Testing
We use the [pytest framework](https://docs.pytest.org/en/latest/) to test recommendations-microservice. The easiest way to run tests it through `nox` with `nox --sessions tests`.
...
FLASK_APP=$(pwd)/recommendations_microservice/app.py poetry run flask run
```
...
```bash
heroku create recommendations_microservice
...
heroku config:set HEROKU_APP_NAME=recommendations-microservice
heroku config:set DD_TAGS=service:recommendations-microservice
```
